...
const commandJson = new SlashCommandBuilder()
  ...
  .addSubcommandGroup((scg) =>
    scg
      .setName("config")
      .setDescription("Manage twitch alert configuration")
      .addSubcommand((sc) =>
        sc.setName("get").setDescription("Get config for twitch alerts"),
      )
      .addSubcommand((sc) =>
        sc
          .setName("set")
          .setDescription("Set config for twitch alerts")
          .addStringOption((o) =>
            o
              .setName("bot-name")
              .setDescription(
                "The name the bot should make the alert posts with",
              ),
          )
          .addStringOption((o) =>
            o
              .setName("bot-avatar-url")
              .setDescription(
                "The url for the avatar the bot should make the alert posts with",
              ),
          )
          .addStringOption((o) =>
            o
              .setName("embed-title-template")
              .setDescription(
                "The title to be used in embed. Must include %s to be replaced with Twitch name.",
              ),
          )
          .addStringOption((o) =>
            o
              .setName("pre-embed-content")
              .setDescription("The message to be posted before the embed."),
          )
          .addChannelOption((o) =>
            o
              .setName("channel")
              .setDescription("The channel to send Twitch alerts to")
              .addChannelTypes(
      ...
      .addSubcommand((sc) =>
        sc
          .setName("unset")
          .setDescription(
            "Unset specific config for twitch alerts. Mark config with true to unset it.",
          )
          .addBooleanOption((o) =>
            o
              .setName("bot-name")
  ...
  .addSubcommandGroup((scg) =>
    scg
      .setName("channels")
      .setDescription("Manage the twitch channels to subscribe to")
      .addSubcommand((sc) =>
        sc
          .setName("add")
          .setDescription(
            "Subscribe to a twitch channel's online and offline events",
          )
          .addStringOption((o) =>
            o
              .setName("twitch-channel")
              .setDescription("The Twitch channel to subscribe to")
              .setRequired(true),
          ),
      )
      .addSubcommand((sc) =>
        sc
          .setName("remove")
          .setDescription(
            "Remove a Twitch channel subscription from this server",
          )
          .addStringOption((o) =>
            o
              .setName("twitch-channel")
              .setDescription("Unsubscribe from this Twitch channel")
              .setRequired(true),
          ),
      )
      .addSubcommand((sc) =>
        sc
          .setName("get")
          .setDescription(
            "Get the twitch channels subscribe to in this server",
          ),
      ),
  )
  .toJSON();
...
twitchAlertStore.setHandleStreamOnline(
  async (guildDatas, stream, twitchChannel) => {
    for (const guildData of guildDatas) {
      const messageConfig = guildData.messageConfig;
      if (!messageConfig.channelToAlert) {
        console.warn(
          "No channel to send alerts to for guild!",
      ...
      if (!messageConfig.webhookToAlert) {
        console.warn(
          "No webhook to send alerts to for guild!",
          guildData.guild.name,
        );
        return;
      }
      ...
      const embed = buildEmbed(
        messageConfig,
        buildMessageData(twitchChannel, stream),
      );
...
const handler = async (interaction: ChatInputCommandInteraction<CacheType>) => {
  ...
  if (!guild) {
    await interaction.reply({
      content:
        "I can't do that! This command only works within a server/guild.",
  ...
  if (subCommandGroup === "config") {
    if (subCommand === "get") {
      ...
      await interaction.reply({
        content: messageConfigToString(messageConfig),
        flags: MessageFlags.Ephemeral,
      });
    } else if (subCommand === "set") {
      const botName = interaction.options.getString("bot-name");
      const botAvatarUrl = interaction.options.getString("bot-avatar-url");
      const embedTitleTemplate = interaction.options.getString(
        "embed-title-template",
      );
      const preEmbedContent =
        interaction.options.getString("pre-embed-content");
      const channel = interaction.options.getChannel("channel", false, [
        ChannelType.GuildText,
        ChannelType.GuildAnnouncement,
      ]);

      await twitchAlertStore.setMessageConfig(
        guild,
        botName,
        botAvatarUrl,
        embedTitleTemplate,
      ...
      const embedTitleTemplate = interaction.options.getBoolean(
        "embed-title-template",
      );
      const preEmbedContent =
        interaction.options.getBoolean("pre-embed-content");
      const channel = interaction.options.getBoolean("channel");

      twitchAlertStore.unsetMessageConfig(
        guild,
        botName,
        botAvatarUrl,
        embedTitleTemplate,
      ...
      await interaction.reply({
        content: "Unset specified config",
        flags: MessageFlags.Ephemeral,
      });
    }
    ...
    if (subCommand === "add") {
      ...
      const twitchChannel =
        await twitchApiClient.users.getUserByName(twitchName);

      if (!twitchChannel) {
        await interaction.followUp({
          content: `Couldn't find twitch user with name ${twitchName}. Double check your spelling?`,
          flags: MessageFlags.Ephemeral,
        });
      } else {
        twitchAlertStore.addTwitchChannelSubscription(
          interaction.guild,
          twitchChannel,
        );
    ...
    if (subCommand === "remove") {
      ...
      await interaction.reply({
        content: `Removed subscription to ${twitchName}`,
        flags: MessageFlags.Ephemeral,
      });
    }
    if (subCommand === "get") {
      const twitchSubscriptions =
        twitchAlertStore.getTwitchChannelSubscriptions(guild);

      if (!twitchSubscriptions || twitchSubscriptions.length === 0) {
        await interaction.reply({
          content: `Not currently subscribed to any Twitch channels`,
          flags: MessageFlags.Ephemeral,
        });
      } else {
        const channels = twitchSubscriptions
          .map((sub) => `${sub.displayName}`)
          .join(", ");
        await interaction.reply({
          content: `Subscribed to ${channels}.`,
          flags: MessageFlags.Ephemeral,
        });
      }
    }
...
function messageConfigToString(
  messageConfig: DiscordMessageConfig | undefined,
): string | undefined {
